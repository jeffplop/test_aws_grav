...
## 1. [IE1/IE6] Diseño de Contenedorización (Frontend y Backend)
Se diseñó una estrategia de contenedorización priorizando la seguridad y el rendimiento para la aplicación de 3 capas.
- **Dockerfile Multi-stage**: Tanto el Frontend como el Backend utilizan etapas `builder`. En el primer stage se descargan dependencias pesadas, y en el stage final solo se copia lo estrictamente necesario para producción. Esto limpia capas intermedias y reduce drásticamente el tamaño de la imagen final.
- **Principio de Menor Privilegio (Usuario Non-root)**: 
  - El Backend de Node.js se ejecuta bajo el usuario nativo `node`.
  - El Frontend (Nginx) se ejecuta usando la imagen `nginxinc/nginx-unprivileged:alpine`, corriendo en el puerto 8080. Si existiese una vulnerabilidad de día cero en Nginx, el atacante no tendría privilegios de `root` en el contenedor, protegiendo el entorno de host de EC2.
...
## 4. [IE4/IE5] Funcionamiento del Contenedor en Instancias EC2
Se logró una integración exitosa en la nube:
- **Red Aislada (Docker Network)**: Frontend, Backend y BD coexisten en la red puente privada `app-network`.
- **Integración Front → Back (Seguridad)**: El Frontend se expone al cliente, pero el Backend y la BD permanecen inalcanzables desde Internet. El Nginx envía las peticiones vía proxy inverso usando nombres de host (`DB_HOST=db`, `BACKEND_HOST=backend`), lo que significa que ni las IPs internas ni públicas están codificadas en duro (hardcodeadas), demostrando flexibilidad ante escalabilidad futura.
